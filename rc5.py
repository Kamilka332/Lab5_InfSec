class RC5:
    def __init__(self, w, R, key, strip_extra_nulls=False):
        ...
        self.key = key  # key.txt (0 to 2040 bits)
    ...
    def __keyAlign(self):
        if self.b == 0:  # key.txt is empty
            self.c = 1
        elif self.b % self.w8:
            self.key += b'\x00' * (self.w8 - self.b % self.w8)  # fill key.txt with \x00 bytes
            self.b = len(self.key)
    ...
    def encryptFile(self, input_file_name, output_file_name):
        with open(input_file_name, 'rb') as inp, open(output_file_name, 'wb') as out:
            run = True
            while run:
    ...
    def decryptFile(self, input_file_name, output_file_name):
        with open(input_file_name, 'rb') as inp, open(output_file_name, 'wb') as out:
            while True:
                text = inp.read(self.w4)
                if not text:
